...
ui <- dashboardPage(
  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "map",
              fluidRow(
                box(title = "Instructions", status = "warning", solidHeader = TRUE, defaultMessageUI("default_message")),
                box(title = "Map", status = "primary", solidHeader = TRUE, mapUI("map"))
              ),
              fluidRow(
                box(title = "Timeline Plot", width = 10, status = "primary", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE, timelineUI("timeline"))
              ),
              fluidRow(
                column(width = 12,
